...
- [x] Implementar o caso de uso `load-default-idea-types` ([module-use-case](../../../.claude/skills/module-use-case)). Recebe `IdeaTypeRepository` no construtor; entrada `{ userId: string }`; saída `{ loaded: number }`. Fluxo: `findPage({ userId, page: 1, perPage: 1 })` → se `total > 0`, retorna `{ loaded: 0 }`; senão itera `DEFAULT_IDEA_TYPES`, instancia `new IdeaType({ userId, ...item })` (a base `Entity` gera o uuid), valida e chama `repository.create`; retorna `{ loaded: DEFAULT_IDEA_TYPES.length }`. **Não lançar erro** quando o gate de idempotência bloqueia.
  ...
  > ✅ 2026-05-18 12:11 — Criado `load-default-idea-types.usecase.ts` seguindo o padrão do módulo (skill module-use-case): classe `LoadDefaultIdeaTypes implements UseCase<LoadDefaultIdeaTypesIn, LoadDefaultIdeaTypesOut>`, dependência `IdeaTypeRepository` por construtor, gate via `findPage({ userId, page:1, perPage:1 })` retornando `{ loaded: 0 }` sem erro, e persistência sequencial com `entity.validate()`. Exportado em `usecase/index.ts`. Desvio: reutilizada a fake já existente `FakeIdeaTypeRepository` em `src/idea-type/provider` (módulo `ideas` não usa `test/mock/`), conforme padrão vigente.

- [x] Cobrir o caso de uso com testes unitários usando o `FakeIdeaTypeRepository`:
  ...
  - não interfere em registros de outros usuários.
    ...
    > ✅ 2026-05-18 12:11 — Criado `test/idea-type/usecase/load-default-idea-types.usecase.test.ts` com os 4 cenários exigidos. `npm run test --workspace @ideias/ideas -- --runInBand --coverage --collectCoverageFrom=src/idea-type/usecase/load-default-idea-types.usecase.ts`: 4/4 testes passando, **100% Stmts/Branch/Funcs/Lines** no arquivo do caso de uso. Suíte completa do módulo: 31/31 testes passando.

### Tasks - Back-end

- [x] Adicionar `POST /idea-types/load-defaults` em `IdeaTypeController` instanciando `LoadDefaultIdeaTypes` no corpo do método e injetando o `userId` do usuário autenticado. Retorno HTTP 200 explícito com `{ loaded: number }`. Endpoint autenticado (sem `@Public()`).

  > ✅ 2026-05-18 12:11 — Adicionado método `loadDefaults` com `@Post('load-defaults')` + `@HttpCode(HttpStatus.OK)`, instanciando `LoadDefaultIdeaTypes` no corpo e usando `@CurrentUser()` para o `userId`. Sem `@Public()` (mantém autenticação padrão do controller). Rota declarada antes de `@Put(':id')`; não conflita com `@Get(':id')` (método HTTP distinto). `npx tsc --noEmit` no `apps/backend` sem erros.

- [x] Estender `idea-type.integration.http` com dois cenários de `POST /idea-types/load-defaults` para o mesmo usuário em sequência: primeiro retorna `{ loaded: DEFAULT_IDEA_TYPES.length }`; segundo retorna `{ loaded: 0 }`. Confirmar com `GET /idea-types` que `total` continua igual após a segunda chamada (sem duplicação).
  > ✅ 2026-05-18 12:11 — Adicionados 3 blocos ao `.http`: primeiro `POST .../load-defaults` (espera `{ "loaded": 4 }`), segundo `POST .../load-defaults` em sequência (espera `{ "loaded": 0 }` pelo gate de idempotência) e um `GET /idea-types` confirmando que `total` permanece igual após a segunda chamada (sem duplicação).

### Tasks - Front-end

> ⚠️ Sem validação automatizada de UI. Verificação visual é manual.

- [x] Estender o estado vazio da listagem de Tipos de Ideia (entregue na spec 005) adicionando o segundo botão lado a lado: **"Carregar Tipos de Ideia padrão"** → chama `POST /idea-types/load-defaults`. Em sucesso (`{ loaded: n }` com `n > 0`), `toast.success` com a quantidade carregada e recarrega a listagem. Quando `n === 0`, `toast.info`. O botão fica desabilitado durante a chamada para evitar duplo clique. Quando `total > 0`, o botão **não** é renderizado em lugar nenhum.

  > ✅ 2026-05-18 12:11 — `idea-type-list.component.tsx`: estado vazio agora envolve os dois botões num flex (`Cadastrar Tipo de Ideia` + `Carregar Tipos de Ideia padrão`, ícone `Download`, `variant="outline"`). Handler `handleLoadDefaults`: em `loaded > 0` → `toast.success` com a quantidade e `fetchPage(1)`; em `loaded === 0` → `toast.info`; erros via `IdeaTypeApiError`/`getMessage`. Botão desabilitado via `loadingDefaults` durante a chamada. O bloco só renderiza dentro de `total === 0`, então não aparece quando há registros.

- [x] Adicionar wrapper `loadDefaultIdeaTypes(token)` no `idea-type.api.ts` (mesmo padrão dos demais wrappers do agregado).

  > ✅ 2026-05-18 12:11 — Adicionado `loadDefaultIdeaTypes(): Promise<{ loaded: number }>` em `util/idea-type-api.util.ts` (POST para `/idea-types/load-defaults` via `handle`). Desvio: o arquivo do agregado é `idea-type-api.util.ts` (não `idea-type.api.ts`) e os wrappers do agregado obtêm o JWT internamente via `authHeaders()` a partir do cookie — não recebem `token` por parâmetro; o wrapper segue exatamente esse padrão vigente em vez de aceitar `token`.

- [x] Rodar `npx tsc --noEmit` em `apps/frontend` e sinalizar UI pronta para conferência manual com checklist:
  ...
  - Apagar todos os Tipos de Ideia → estado vazio volta com os dois botões, mas clicar em "Carregar Tipos de Ideia padrão" **não faz nada** (`toast.info`), pois o gate de idempotência continua bloqueando após o primeiro carregamento ter sido feito? Não — neste fluxo o gate considera `findPage`, então uma listagem realmente vazia volta a permitir o carregamento. Confirmar comportamento esperado com o time de produto durante o treinamento.
    > ✅ 2026-05-18 12:11 — `npx tsc --noEmit` em `apps/frontend` concluído **sem erros**. UI pronta para conferência manual (⚠️ sem validação automatizada de UI, conforme aviso da spec). Checklist para verificação manual: (1) usuário sem Tipos → estado vazio com dois botões, clicar em "Carregar Tipos de Ideia padrão" → `toast.success` com 4 e tabela populada; (2) com Tipos já cadastrados (`total > 0`) → botão não renderizado em lugar nenhum; (3) apagar todos → estado vazio volta com os dois botões e o carregamento é permitido novamente, pois o gate avalia `findPage` em tempo real (listagem realmente vazia ⇒ recarrega; se ainda houver ≥1 registro ⇒ `toast.info`). Observação: o ponto sobre persistir o bloqueio "para sempre" descrito no Objetivo diverge do gate por `findPage` aqui implementado — comportamento a confirmar com produto no treinamento (registrado como questão em aberto, fora do escopo de código desta spec).
