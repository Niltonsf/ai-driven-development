...
- [ ] Criar a entidade `Resource` em `modules/ideas/src/idea/model/resource.entity.ts` com a skill [module-entity](../../../.claude/skills/module-entity). Estado:
...
- [ ] Estender o estado da entidade `Idea` (`modules/ideas/src/idea/model/idea.entity.ts`) com o campo `resources: Resource[]` (default `[]` quando o construtor recebe `undefined`). Adicionar getter `resources`. Atualizar `Idea.validate()` para:
...
- [ ] Atualizar a interface do repositório `IdeaRepository` (`modules/ideas/src/idea/provider/idea.repository.ts`):
...
- [ ] Atualizar o caso de uso `save-idea` (`modules/ideas/src/idea/usecase/save-idea.usecase.ts`):
...
- [ ] Atualizar o caso de uso `delete-idea` (`modules/ideas/src/idea/usecase/delete-idea.usecase.ts`):
...
- [ ] Cobrir as mudanças com testes unitários, usando os fakes do módulo. Cenários mínimos:
...
- [ ] Atualizar `apps/backend/src/modules/ideas/idea.controller.ts`:
...
- [ ] Atualizar `apps/backend/src/modules/ideas/idea.integration.http` cobrindo os fluxos novos:
...
- [ ] Criar o componente `apps/frontend/src/modules/ideas/components/idea-resources-input.component.tsx`. Responsabilidades:
...
- [ ] Atualizar `.spec/memory/dominio.md` registrando que o conceito Recurso, no modelo, é uma entidade interna do agregado `idea` com campos `type` (union, MVP `'text'`), `content` e `position`, e que tipos adicionais entram em specs futuras estendendo o union sem mudar a estrutura plana.
