...
> - `{{modulo}}` — módulo de negócio onde o cadastro vive. No Banco de Ideias só existem dois módulos: `auth` e `business` (ver [Módulos](../../memory/modulos.md)). **Assume-se que o módulo já existe**; este template não cria módulos novos.
...
- Módulo de negócio: `{{modulo}}` (já existente, deve ser `auth` ou `business`), agregado `{{entidade}}`.
...
- Conceitos do domínio (Ideia, Tipo de Ideia, Prompt, Recurso, Resultado) seguem [Domínio](../../memory/dominio.md). Quando o CRUD envolver geração via IA, considerar [Processamento com IA](../../memory/processamento-ia.md).
...
- [Domínio](../../memory/dominio.md)
- [Módulos](../../memory/modulos.md)
- [Processamento com IA](../../memory/processamento-ia.md)
- [Glossário](../../memory/glossario.md)
...
- Se a entidade fizer parte do núcleo do Banco de Ideias (ex.: `idea`, `resource`, `idea-type`, `result`), garantir que ela viva no módulo `business` e que respeite as regras de domínio descritas em [Domínio](../../memory/dominio.md).
