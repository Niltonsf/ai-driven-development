...
### 3d. Copiar ilustrações estáticas para `public/`

Componentes da `shared/` (ex.: `empty-list-state.tsx`) referenciam SVGs em `/illustrations/`
servidos pelo Next.js a partir do `public/` do destino. Copiar a pasta embarcada na skill:

```bash
mkdir -p "{DEST}/public/illustrations"
cp -R "{SKILL_DIR}/assets/public/." "{DEST}/public/"
echo "ilustrações copiadas para public/"
```

> Sem este passo, o componente carrega mas a `<Image>` quebra em runtime
> (`/illustrations/empty-list-dark.svg` retorna 404). Os SVGs vivem em
> `{SKILL_DIR}/assets/public/illustrations/` — adicionar novos lá quando criar
> componentes que precisem de outros assets estáticos.

### 3e. Verificar alias de import
...
ls "{DEST}/public/illustrations/empty-list-dark.svg"
```
