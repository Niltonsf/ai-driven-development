...
export type GroupedBarChartSeries<TData extends ChartDatum> = {
...
type GroupedBarChartProps<TData extends ChartDatum> = {
  data: TData[];
  xKey: keyof TData & string;
  /** One bar per series for each item of `data`, side by side, in this order. */
  series: GroupedBarChartSeries<TData>[];
  height?: number;
  className?: string;
...
/** Grouped (never stacked) bars, with the same grid, axes, tooltip and legend of `ComposedBarLineChart`. */
export function GroupedBarChart<TData extends ChartDatum>({
  data,
  xKey,
  series,
  height = 320,
