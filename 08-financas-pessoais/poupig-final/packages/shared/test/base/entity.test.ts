...
describe('Entity', () => {
  ...
  describe('array and date comparisons in diff', () => {
    ...
    test('should preserve non-plain class instances by reference through deep clone', () => {
      class Money {
      ...
      const entity = TestEntity.create({ number: 1, obj: money });

      const result = entity.cloneProps({ number: 2 });

      // A class instance is neither a Date, an Array, nor a plain object, so
      // deepClone returns it untouched — keeping its prototype (and methods).
      expect(result.props.obj).toBe(money);
      expect((result.props.obj as Money).double()).toBe(20);
    });
  });
