# instagram-lista-videos-csv (MCP)
...
| `outputDir` | string | não         | Pasta-base de destino (ex.: pasta do markdown). O CSV vai para `<outputDir>/dados/lista.csv`. |
...
> Dica para receitas/prompts: passe `outputDir` = pasta do markdown que está
> orquestrando o trabalho. O MCP cria a subpasta `dados/` dentro dela e grava
> `dados/lista.csv` — assim a lista (e os vídeos baixados depois pelo MCP
> `instagram-top-videos-download`, que sempre grava na mesma pasta do CSV)
> ficam organizadinhos junto do prompt em `dados/`.
...
```bash
cd resources/mcps/instagram-lista-videos-csv
npm install
```
...
```bash
claude mcp add instagram-lista-videos-csv -- node /caminho/abs/resources/mcps/instagram-lista-videos-csv/index.js
```
...
{
  "mcpServers": {
    "instagram-lista-videos-csv": {
      "command": "node",
      "args": ["/caminho/abs/resources/mcps/instagram-lista-videos-csv/index.js"]
    }
  }
}
```

Por padrão o CSV é gravado em `recursos/videos/<conta>/lista.csv` (caminho
relativo à raiz do projeto, resolvido a partir da pasta do MCP). A pasta de
saída pode ser sobrescrita de duas formas:

1. **Por chamada** — passando `outputDir` no `tools/call` (recomendado quando
   o MCP é orquestrado por um markdown/receita: passe a pasta desse markdown).
   O CSV vai para `<outputDir>/dados/lista.csv` (a subpasta `dados` é criada
   automaticamente).
2. **Por processo** — via variável de ambiente `OUTPUT_DIR` (afeta todas as
   chamadas naquele processo; mantém a subpasta por conta).
