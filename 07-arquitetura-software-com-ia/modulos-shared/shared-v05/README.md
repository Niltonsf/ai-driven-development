...
```typescript
interface CreateRepository<T>    { create(entity: T, tx?): Promise<Result<void>>  }
interface UpdateRepository<T>    { update(entity: T, tx?): Promise<Result<void>>  }
interface FindByIdRepository<T>  { findById(id: string): Promise<Result<T>>       }
interface DeleteRepository       { delete(id: string, tx?): Promise<Result<void>> }
...
```typescript
interface PaginatedInputDTO    { page: number; pageSize: number }
interface PaginationMetaDTO    { page: number; pageSize: number; total: number; totalPages: number }
interface PaginatedResultDTO<T>{ data: T[]; meta: PaginationMetaDTO }
```
