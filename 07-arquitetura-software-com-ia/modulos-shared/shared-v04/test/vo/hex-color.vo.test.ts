...
describe('HexColor', () => {
  ...
  test('should throw when constructing directly with invalid hex color', () => {
    expect(() => new HexColor('blue')).toThrow('hexcolor.invalid');
  });

  test('should throw when constructing directly with null value', () => {
    expect(() => new HexColor(null as any)).toThrow('hexcolor.invalid');
  });

  test('should return false from isValid for null or non-string', () => {
    expect(HexColor.isValid(null as any)).toBe(false);
    expect(HexColor.isValid('' as any)).toBe(false);
  });
