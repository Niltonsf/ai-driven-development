...
describe('Email', () => {
  ...
  test('should expose username as alias for local', () => {
    const email = Email.create('john.doe@pharmacore.com');

    expect(email.username).toBe('john.doe');
  });

  test('should throw when constructing directly with invalid email', () => {
    expect(() => new Email('invalid')).toThrow('email.invalid');
  });

  test('should return false from isValid for null or non-string', () => {
    expect(Email.isValid(null as any)).toBe(false);
    expect(Email.isValid('' as any)).toBe(false);
  });
});
