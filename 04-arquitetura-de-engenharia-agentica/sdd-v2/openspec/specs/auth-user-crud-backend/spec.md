...
O backend SHALL incluir `apps/backend/src/modules/auth/user.controller.ts` expondo o CRUD em `/users` (criar, atualizar, excluir, obter por id e listar paginado). Todos os endpoints SHALL ser autenticados (sem `@Public()`).
...
O backend SHALL incluir `apps/backend/src/modules/auth/user.integration.http` (Rest Client) cobrindo os fluxos do CRUD e os principais casos de erro (ex.: criação válida, atualização válida, atualização sem `password`, exclusão de usuário existente, obter por id, listagem paginada, exclusão de usuário inexistente, acesso sem JWT).
